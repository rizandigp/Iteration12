...
float4 PS( PS_INPUT input) : SV_Target
{
	...
	Specmap.x = 0.02f;
	Specmap.y = pow(SpecularParams.y,Specmap.y);

	float3 diff, spec;
	float3 lightvec;

	float normalization = (Specmap.y + 2.0f )/(2.0f);

	// Omni lights
	[unroll] for(int i=0; i<2; i++)
	{
	lightvec = vPointLightPos[i].xyz-input.WSPos.xyz;
	diff = saturate( dot( normalize( lightvec ), normals ))*vPointLightColor[i].xyz/(length(lightvec)*length(lightvec)+1);
	diff -= diff*length(lightvec)/vPointLightPos[i].a;
	spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), Specmap.y)*Specmap.x*vPointLightColor[i].xyz/(length(lightvec)*length(lightvec)+1);
	spec *= normalization;
	...
	if (!(ShadowTexC.x <= 0.0f || ShadowTexC.x >= 1.0f || ShadowTexC.y <= 0.0f || ShadowTexC.y >= 1.0f))
	{
		...
		diff = saturate( dot( normalize( lightvec ), normals ))*vSpotLightColor[0].xyz/(length(lightvec)*length(lightvec)+1);
		diff -= diff*length(lightvec)/vSpotLightPos[0].a;
		spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), Specmap.y)*Specmap.x*vSpotLightColor[0].xyz/(length(lightvec)*length(lightvec)+1);
		spec *= normalization;
...

	{
		...
		diff = saturate( dot( normalize( lightvec ), normals ))*vSpotLightColor[1].xyz/(length(lightvec)*length(lightvec)+1);
		diff -= diff*length(lightvec)/vSpotLightPos[1].a;
		spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), Specmap.y)*Specmap.x*vSpotLightColor[1].xyz/(length(lightvec)*length(lightvec)+1);
		spec *= normalization;
