...
PSOut PS( PS_INPUT input) : SV_Target
{
	...
	float4 specular = txSpecular.Sample( samLinear, input.Tex ).rgba;//*SpecularParams.x;
	float gloss = txGlossiness.Sample( samLinear, input.Tex ).r*fGlossinessMultiplier;
	float ao = 1.0f-((1.0f-pow(txAO.Sample( samLinear, input.Tex ).r,fAOExponent))*fAOIntensity);
	...
	float4 fresnel = pow( 1.0f - saturate( dot( viewvector, normals ) ), 5.0f );
    fresnel *= ( 1.0f - specular );
    fresnel += specular;

	PSOut output;
	output.backbuffer = ONEDIVPI*pow(txDiffuse.Sample( samLinear, input.Tex ),GAMMATOLINEAR)*ao*txIBL.SampleLevel( samLinear, normals.xzy, 8.5f );
	output.backbuffer += ONEDIVPI*ao*fresnel*txIBL.SampleLevel( samLinear, reflectionvector, (1.0f-gloss)*10.0f );
	...
	output.gbuffer2.xyz = specular.rgb;
