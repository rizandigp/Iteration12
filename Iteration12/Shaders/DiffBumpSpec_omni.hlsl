...
float4 PS( PS_INPUT input) : SV_Target
{
	...
	spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), SpecularParams.y*Specmap.y)*SpecularParams.x*Specmap.x*vPointLightColor[i].xyz/(length(lightvec)*length(lightvec)+1);
