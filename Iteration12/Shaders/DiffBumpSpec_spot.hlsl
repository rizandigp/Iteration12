...
float4 PS( PS_INPUT input) : SV_Target
{
	...
	if (!(ShadowTexC.x <= 0.0f || ShadowTexC.x >= 1.0f || ShadowTexC.y <= 0.0f || ShadowTexC.y >= 1.0f))
	{
		...
		spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), SpecularParams.y*Specmap.y)*SpecularParams.x*Specmap.x*vSpotLightColor[0].xyz/(length(lightvec)+1);
...

	{
		...
		spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), SpecularParams.y*Specmap.y)*SpecularParams.x*Specmap.x*vSpotLightColor[1].xyz/(length(lightvec)+1);
