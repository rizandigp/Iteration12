...
float4 PS( PS_INPUT input) : SV_Target
{
	...
	float3 l_diffuse =  float3( 0.0f, 0.0f, 0.0f );
	...
	if (!(ShadowTexC.x <= 0.0f || ShadowTexC.x >= 1.0f || ShadowTexC.y <= 0.0f || ShadowTexC.y >= 1.0f || input.CSPos[0].w < 0))
	{
		...
		spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), SpecularParams.y)*SpecularParams.x*diff*vSpotLightColor[0].xyz/(length(lightvec)*length(lightvec)+1);
...

	{
		...
		diff = saturate( dot( normalize( lightvec ), normals ));
		spec = pow(saturate( dot( normalize( reflect( normalize( -lightvec ), normals ) ), viewdir )), SpecularParams.y)*SpecularParams.x*diff*vSpotLightColor[1].xyz/(length(lightvec)*length(lightvec)+1);
	...
	//return input.CSPos[0].xyzw;
	//return vSpotLightColor[1];
